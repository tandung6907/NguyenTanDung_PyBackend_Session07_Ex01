...
def run_tests():
    print("=" * 70)
    print("TEST CASE 1: order_id = 999 (không tồn tại) -> phải là 404")
    print("=" * 70)
    response = client.get("/orders/999")
    print(f"Status code: {response.status_code}")
    print(f"Body: {response.json()}")
    assert response.status_code == 404, "SAI: phải trả về 404 Not Found"
    assert response.json()["detail"] == "Order not found"
    print("PASS: Trả đúng 404 Not Found\n")

    print("=" * 70)
    print("TEST CASE 2: order_id = 1 (tồn tại) -> phải là 200, không lộ field mật")
    print("=" * 70)
    ...
    assert "profit_margin" not in body, "LỖI: profit_margin vẫn bị lộ"
    assert "supplier_id" not in body, "LỖI: supplier_id vẫn bị lộ"
    assert body == {
        "id": 1,
        "customer_name": "Nguyen Van A",
        "total_amount": 1500000.0,
    }
    print("PASS: Không lộ profit_margin và supplier_id\n")

    print("=" * 70)
    print("TEST CASE 3: order_id = 2 (tồn tại) -> kiểm tra chéo dữ liệu khác")
    print("=" * 70)
    ...
    print(f"Body: {body}")
    assert response.status_code == 200
    assert "profit_margin" not in body
    assert "supplier_id" not in body
    assert body["customer_name"] == "Tran Thi B"
    print("PASS: Dữ liệu công khai đúng, không lộ field mật\n")
